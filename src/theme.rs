...
impl Theme {
    pub fn closest_from_color(color: Rgb<u8>) -> (Theme, f32) {
        ...
        // Calculate color differences for all themes
        let themes_with_distances: Vec<(Theme, f32)> = Self::iter()
            .map(|theme| (theme.clone(), color_difference((theme.primary(), hsl))))
            .collect();

        // Find the theme with the minimum color difference
        // This should never fail since we have a static list of themes,
        // but we'll handle the case where the list might be empty anyway
        themes_with_distances
            .into_iter()
            .min_by(|a, b| a.1.total_cmp(&b.1))
            .unwrap_or_else(|| {
                // Default to Lotus theme with a large distance if no themes are available
                // This should never happen with the current implementation
                eprintln!("Warning: No themes available for comparison");
                (Theme::Lotus, f32::MAX)
            })
    }
