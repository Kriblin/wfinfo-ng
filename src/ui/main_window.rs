...
impl eframe::App for MainApp {
    fn logic(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        let did_update = if let Ok((rewards, _, _)) = self.reward_receiver.try_recv() {
            self.last_capture_state.update(rewards.clone());
            self.overlay_state.set_rewards(rewards);
            true
        } else {
            false
        };
        ...
        if did_update {
            self.overlay_active = self.overlay_enabled;
            ctx.request_repaint();
            debug!(
    ...
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        egui::Frame::central_panel(ui.style()).show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                ui.separator();
                if ui
                    .checkbox(&mut self.overlay_enabled, "Overlay enabled")
                    .changed()
            ...
            ui.horizontal(|ui| {
                ...
                if self.capture_mode == CaptureMode::Window && ui.button("Refresh").clicked() {
                    let current_title = self.window_title_state.get();
                    find_window_by_title(&current_title, &self.window_capture_state);
                } else if self.capture_mode == CaptureMode::Monitor
            ...
            egui::CollapsingHeader::new("Last capture result")
                .default_open(true)
                .show(ui, |ui| match self.last_capture_state.rewards() {
                    Some([]) => {
                        ui.label("The last capture did not contain any rewards.");
                    }
                    Some(rewards) => draw_reward_list(ui, rewards),
                    None => {
                        ui.label("No capture result yet.");
                    }
                });
            ...
            if self.overlay_enabled && self.overlay_active {
